	...
	public:
		...
		auto LoadText(const std::string& text) -> bool {
			...
			if (this->font_ == nullptr) {
				Log::LogOut("Font not loaded! Cannot create text.", LogLevel::HL_ERROR);
				return false;
			}
			...
			Log::LogOut("Text created: " + text, LogLevel::HL_INFO);
			return true;
		}
		...
		auto SetToFullscreen() -> void {
				...
				Log::LogOut("Set to Fullscreen Mode!", LogLevel::HL_INFO);
			}
		...
		auto SetToWindowed() -> void {
				...
				Log::LogOut("Set to Windowed Mode!", LogLevel::HL_INFO);
			}
		...
		void SetupFadeCallbacks() {
				engine::MusicFade::GetInstance().SetCallback(
					[](engine::MusicFade::FadeState state,
					   const std::string& fromId, const std::string& toId) {
						switch (state) {
							case engine::MusicFade::FadeState::FadingOut:
							Log::LogOut("Fading out: " + fromId, LogLevel::HL_INFO);
							break;
						case engine::MusicFade::FadeState::FadingIn:
							Log::LogOut("Fading in: " + toId, LogLevel::HL_INFO);
							break;
						case engine::MusicFade::FadeState::Completed:
							Log::LogOut("Fade completed: " + fromId + " -> " + toId, LogLevel::HL_INFO);
							break;
						default:
							break;
					}
		...
		auto CreateImGuiInterface() -> void {
				// 1. 显示一个简单的窗口
				{
					...
					if (ImGui::Button("Switch Background")) {
						bool success = engine::MusicFade::GetInstance().Switch("bg1", "bg2", 3.0f);
						if (success) {
							Log::LogOut("Started fade transition from bg1 to bg2", LogLevel::HL_INFO);
						}
					}
